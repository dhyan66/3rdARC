  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const photographerResponse = await fetch('/photographer.json');
        if (!photographerResponse.ok) throw new Error(`Failed to load photographer: ${photographerResponse.status}`);
        ...
        const seriesPromises = seriesSlugs.map(async (slug) => {
          ...
          if (!response.ok) throw new Error(`Failed to load series ${slug}: ${response.status}`);
